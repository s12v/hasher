 ...
 * day-of-week), lists, ranges, steps, month/day names, @shortcuts.
 *
...
var cron = {
  ...
  /**
   * Returns [{ items: [{lo, hi, step, star, single}], values: {n: true}, star: bool }] per field; throws on error
   */
  parse : function (expr) {
  ...
  describe : function (expr) {
    ...
    // "At 04:05" when both are fixed single values
    if (minute.items.length == 1 && minute.items[0].single && hour.items.length == 1 && hour.items[0].single) {
      var h = hour.items[0].lo, m = minute.items[0].lo;
      out = "At " + (h < 10 ? "0" : "") + h + ":" + (m < 10 ? "0" : "") + m;
    } else {
      out = "At " + this.describeField(minute, this.FIELDS[0]);
      if (!(hour.items.length == 1 && hour.items[0].star && hour.items[0].step == 1)) {
        out += " past " + this.describeField(hour, this.FIELDS[1]);
      }
    }
    ...
    var days = [];
    if (!isEvery(dom)) {
      days.push("on " + this.describeField(dom, this.FIELDS[2]));
    }
    if (!isEvery(dow)) {
      days.push("on " + this.describeField(dow, this.FIELDS[4]));
    }
    if (days.length > 0) {
      out += " " + days.join(" and ");
    }
    if (!isEvery(month)) {
      out += " in " + this.describeField(month, this.FIELDS[3]);
    }
    return out + ".";
  },
  ...
  next : function (expr, from, count) {
    ...
    var minute = f[0].values, hour = f[1].values, dom = f[2].values, month = f[3].values, dow = f[4].values;
    var both = !f[2].star && !f[4].star;
    var result = [];
    var d = new Date(from.getTime());
    d.setSeconds(0, 0);
    d.setMinutes(d.getMinutes() + 1);
    // 5 years of minutes is the search horizon; "0 0 30 2 *" must terminate
